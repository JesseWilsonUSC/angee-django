...
MESSAGING_ACCESS_MODELS = (Vendor, Integration, Channel, Fragment, Thread, Message)
...
def test_channel_owner_reaches_threads_and_messages(messaging_access_schema: str) -> None:
    ...
    created = _create_missing_tables(MESSAGING_ACCESS_MODELS)
    try:
        ...
        _clear_model_tables(MESSAGING_ACCESS_MODELS)
        if created:
